...
function readPort(value: string | undefined): number {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535 || port === 3000) {
    throw new Error("KNOWHERE_GAME_SERVER_PORT must be a valid port other than client port 3000.");
  }
  return port;
}
...
export function readHostServerConfig(environment = process.env): HostServerConfig {
  ...
  return {
    host: environment.KNOWHERE_GAME_SERVER_HOST?.trim() || "127.0.0.1",
    port: readPort(environment.KNOWHERE_GAME_SERVER_PORT),
